# Script PowerShell para aplicar migração de triagem via Docker
# Execute: .\migrar-triagem.ps1

Write-Host "🏥 === MIGRAÇÃO DE TRIAGEM VIA DOCKER ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "🚀 Aplicando migração de triagem..." -ForegroundColor Green
...
docker-compose exec db psql -U postgres -d prontuario -c $migrationSQL
...
docker-compose exec db psql -U postgres -d prontuario -c $verificationSQL
