...
if (!process.env.DATABASE_URL) {
  ...
  console.error('💡 Configure a DATABASE_URL para o banco de produção antes de executar este script.');
  process.exit(1);
}

// Confirmação de segurança
console.log('⚠️  AVISO: Este script irá modificar o banco de PRODUÇÃO!');
console.log('🔗 Conectando em:', process.env.DATABASE_URL.replace(/:[^:@]*@/, ':***@')); // Oculta senha no log
console.log('⏱️  Iniciando em 3 segundos...\n');

// Executar após 3 segundos (tempo para cancelar se necessário)
setTimeout(() => {
