          ...
          <div class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
              ...
              <tbody class="divide-y divide-gray-200">
                <tr *ngFor="let atendimento of atendimentos; let i = index"
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {{ formatarData(atendimento.data_hora_atendimento) }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full"
                          [ngClass]="{
                            'bg-red-100 text-red-800 border border-red-200': atendimento.classificacao_risco === 'vermelho',
                            'bg-yellow-100 text-yellow-800 border border-yellow-200': atendimento.classificacao_risco === 'amarelo',
                            'bg-green-100 text-green-800 border border-green-200': atendimento.classificacao_risco === 'verde',
                            'bg-blue-100 text-blue-800 border border-blue-200': atendimento.classificacao_risco === 'azul',
                            'bg-gray-100 text-gray-800 border border-gray-200': !atendimento.classificacao_risco
                          }">
                      {{ atendimento.classificacao_risco || 'N/A' }}
                    </span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                    {{ atendimento.status }}
            ...
            <div *ngIf="atendimentos.length === 0" class="text-center py-12">
              ...
              <p class="text-gray-500">Não há atendimentos registrados para este período.</p>
            </div>
          </div>
        </div>
